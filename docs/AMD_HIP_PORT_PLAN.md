...
**Status (local `hip` branch):** Phase 0 complete. Phase 1A/1B largely done (~46/88
...
`gqa_causal`). `./h3_hip_bf16_tests` runs 33 CPU-oracle checks without MLX
fixtures. Phase 5 int8: `quantize_weight_int8`, naive `linear_int8_bf16`,
reference `mlp_int8_bf16`, `linear_int8_head_major_bf16`, and
`grouped_qkv_linear_rope_int8` (NAX 128×128 fast paths still TODO).
