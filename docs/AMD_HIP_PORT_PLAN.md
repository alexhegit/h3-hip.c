...
**Status (local `hip` branch):** Phase 0 complete. Phase 1A/1B largely done (~70/88
...
`./h3_hip_bf16_tests` runs 59 CPU-oracle checks without MLX fixtures. Phase 5
...
128×128 kernels still TODO. Remaining audio/VAE conv and int8 fast-path kernels
remain stubbed.
