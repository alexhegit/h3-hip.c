...
**Status (local `hip` branch):** Phase 0 complete. Phase 1A/1B largely done (~48/88
...
`gqa_causal`). `./h3_hip_bf16_tests` runs 36 CPU-oracle checks without MLX
fixtures. Phase 5 int8 and reference NAX paths (`mlp_nax_bf16`, grouped
quantize, int8 MLP/QKV) are in place; tiled NAX 128×128 kernels still TODO.
