...
**Status (local `hip` branch):** Phase 0 complete. Phase 1A/1B largely done (~72/88
...
`weight_norm_f32`, `geglu_f32`, `gate_f32`, `adaln_f32`, `qkv_rope_f32`,
`sdpa_f32`, `video_qkv_rope_f32`, `conv1d_f32`, `conv1d_stride_f32`,
`conv_transpose1d_f32`, `alias_free_snake_f32`, `snake1d_f32`,
`audio_qkv_split_f32`, `sdpa_causal_f32`).
`./h3_hip_bf16_tests` runs 61 CPU-oracle checks without MLX fixtures. Phase 5
...
128×128 kernels still TODO. Remaining audio pool/VAE conv and int8 fast-path
kernels remain stubbed.
