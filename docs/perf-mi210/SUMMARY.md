...
| Preset | Halo v0.9.0 | MI210 start | After hipBLAS | Device VAE/DiT | BF16 linear | **CDNA flash** | **`main` 2026-09-02** |
|--------|-------------|-------------|---------------|----------------|------------:|---------------:|----------------------:|
| fox-s2 E2E | 83–87 s | 241 s | 92 s | 17 s | 11.28 s | **10.87 s** | **10.78 s** (`main` BF16) |
| fox-fast E2E | 95 s | — | 232 s | 45 s | 21.01 s | **18.56 s** | **18.24 s** |
| 15 s / 362f E2E | 45.0 min | — | — | — | 50 min 14.58 s | **12 min 33.29 s** | **12 min 11 s** (480 px VAE) |
| 15 s + `--token-reduction` | **28.2 min** | — | — | — | — | **8 min 21 s** (500.81 s; quality trade) | **8 min 21 s** (501.1 s all-opts) |
...
8. **gfx90a f32 d64 VAE flash** (16×32). VAE SDPA micro 103→15 ms; fox-fast
   ...
   slower and rejected.
9. **`main` 2026-09-02 retune** (480 px VAE tiles): fox-s2 **10.78 s**, fox-fast
   **18.24 s**, 15 s BF16 **12 min 11 s** (peak 41.2 GiB / 64 GiB). INT8 DiT is
   VRAM-only on this card. All-opts 15 s **8 min 21 s** (token reduction).
